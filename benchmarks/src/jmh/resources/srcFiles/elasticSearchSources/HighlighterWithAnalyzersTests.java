    ...
    public void testMultiPhraseCutoff() throws IOException {
        ...
        prepareIndex("test").setId("1")
            .setSource(
                ...
                "Test: http:
                    + "http:
                    + "a test for highlighting feature Test: http:
                    + "http:
                    + "http:
            )
            .get();
        refresh();
        assertResponse(
            prepareSearch().setQuery(matchPhraseQuery("body", "Test: http:
                .highlighter(new HighlightBuilder().field("body").highlighterType("fvh")),
            response -> assertHighlight(response, 0, "body", 0, startsWith("<em>Test: http:
        );
