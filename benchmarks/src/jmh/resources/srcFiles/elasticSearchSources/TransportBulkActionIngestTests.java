...
public class TransportBulkActionIngestTests extends ESTestCase {
    ...
    public void testIngestLocal() throws Exception {
        ...
        assertTrue(redirectPredicate.getValue().test(WITH_FAILURE_STORE_ENABLED + "-1")); 
        assertFalse(redirectPredicate.getValue().test(WITH_DEFAULT_PIPELINE)); 
        assertFalse(redirectPredicate.getValue().test("index")); 
        redirectHandler.getValue().apply(2, WITH_FAILURE_STORE_ENABLED + "-1", exception); 
