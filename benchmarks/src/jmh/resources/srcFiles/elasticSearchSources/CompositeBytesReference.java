...
public final class CompositeBytesReference extends AbstractBytesReference {
    ...
    private static BytesReference ofMultiple(BytesReference[] references) {
        ...
        return new CompositeBytesReference(
            ...
            offsets,
            offset,
            ramBytesUsed + (Integer.BYTES * offsets.length + RamUsageEstimator.NUM_BYTES_ARRAY_HEADER) 
                + (references.length * RamUsageEstimator.NUM_BYTES_OBJECT_REF + RamUsageEstimator.NUM_BYTES_ARRAY_HEADER) 
                + Integer.BYTES 
                + Long.BYTES
        );
    }
