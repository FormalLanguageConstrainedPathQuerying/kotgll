...
public final class HdfsClientThreadLeakFilter implements ThreadFilter {
    ...
    public boolean reject(Thread t) {
        return t.getName().contains(OFFENDING_THREAD_NAME)
            ...
            || t.getName().startsWith("SSL Certificates Store Monitor") 
            || t.getName().startsWith("GcTimeMonitor") 
            || t.getName().startsWith("Command processor") 
            || t.getName().startsWith("ForkJoinPool-"); 
    }
}
