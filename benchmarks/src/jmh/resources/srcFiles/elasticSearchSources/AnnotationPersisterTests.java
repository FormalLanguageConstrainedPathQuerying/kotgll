...
public class AnnotationPersisterTests extends ESTestCase {
    ...
    public void testPersistMultipleAnnotationsWithBulk_Failure() {
        doAnswer(withResponse(new BulkResponse(new BulkItemResponse[] { bulkItemFailure("1"), bulkItemFailure("2") }, 0L)))  
            .doAnswer(withResponse(new BulkResponse(new BulkItemResponse[] { bulkItemSuccess("1"), bulkItemFailure("2") }, 0L)))  
            .doAnswer(withResponse(new BulkResponse(new BulkItemResponse[] { bulkItemFailure("2") }, 0L)))  
            .when(client)
            .execute(eq(TransportBulkAction.TYPE), any(), any());
        ...
        for (int i = 2; i < bulkRequests.size(); ++i) {
            assertThat(bulkRequests.get(i).numberOfActions(), equalTo(1));  
        }
    }
