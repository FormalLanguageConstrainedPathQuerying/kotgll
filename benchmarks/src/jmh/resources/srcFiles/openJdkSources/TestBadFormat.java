...
class BadCheckTest {
    ...
    @Check(test = "test4")
    public void wrongReturnParameter4(TestInfo info, int x) {} 
...
class BadIRAnnotations {
    ...
    @IR(failOn = IRNode.CALL, applyIf = {"TLABRefillWasteFraction", "48"}) 
    @IR(failOn = IRNode.CALL, applyIf = {"TLABRefillWasteFraction", "48.5"})
    public void wrongFlagValueLongFlag() {}

    @Test
    @FailCount(3)
    @IR(failOn = IRNode.CALL, applyIf = {"UseTLAB", "true"}) 
    ...
    @IR(failOn = IRNode.CALL, applyIf = {"CompileThresholdScaling", "48"}) 
    @IR(failOn = IRNode.CALL, applyIf = {"CompileThresholdScaling", "48.5"}) 
    public void wrongFlagValueDoubleFlag() {}

    @Test
    @NoFail
    @IR(failOn = IRNode.CALL, applyIf = {"ErrorFile", "true"}) 
    @IR(failOn = IRNode.CALL, applyIf = {"ErrorFile", "SomeString"}) 
    @IR(failOn = IRNode.CALL, applyIf = {"ErrorFile", "48"}) 
    @IR(failOn = IRNode.CALL, applyIf = {"ErrorFile", "48.5"}) 
...
class BadIRNodeForPhase {
    ...
    @IR(failOn = IRNode.CHECKCAST_ARRAY, phase = CompilePhase.PRINT_OPTO_ASSEMBLY) 
    @IR(failOn = IRNode.FIELD_ACCESS, phase = CompilePhase.FINAL_CODE)
    @IR(failOn = IRNode.FIELD_ACCESS, phase = {CompilePhase.PRINT_OPTO_ASSEMBLY, CompilePhase.DEFAULT}) 
    ...
    @IR(failOn = IRNode.LOOP, phase = CompilePhase.FINAL_CODE) 
    @IR(failOn = IRNode.COUNTED_LOOP, phase = CompilePhase.FINAL_CODE) 
    @IR(failOn = IRNode.COUNTED_LOOP_MAIN, phase = CompilePhase.FINAL_CODE) 
    public void loops() {}

    @Test
    @FailCount(6)
    @IR(failOn = IRNode.LOAD_VECTOR_I, phase = CompilePhase.BEFORE_REMOVEUSELESS) 
    @IR(failOn = IRNode.STORE_VECTOR, phase = CompilePhase.BEFORE_REMOVEUSELESS) 
    @IR(failOn = IRNode.VECTOR_CAST_B2I, phase = CompilePhase.BEFORE_REMOVEUSELESS) 
    @IR(failOn = IRNode.LOAD_VECTOR_I, phase = CompilePhase.BEFORE_MATCHING) 
    @IR(failOn = IRNode.STORE_VECTOR, phase = CompilePhase.BEFORE_MATCHING) 
    @IR(failOn = IRNode.VECTOR_CAST_B2I, phase = CompilePhase.BEFORE_MATCHING) 
