        ...
        for (String expected : alternatives) {
            ...
            if (lengthDifference == 0) {
                if (expected.equals(s)) {
                    ...
                    if (s.length() < 6) {
                        spellingError = diff(expected, s) < 2; 
                    } else {
                        spellingError = diff(expected, s) < 3; 
                    }
                }
            }
            if (lengthDifference == 1) {
                spellingError = inSequence(expected, s); 
            }
            if (lengthDifference == -1) {
                spellingError = inSequence(s, expected); 
            }
            if (spellingError) {
                return expected;
            }
