...
abstract class MethodHandleImpl {
    ...
    static Object valueConversion(Class<?> src, Class<?> dst, boolean strict, boolean monobox) {
        assert(!VerifyType.isNullConversion(src, dst, /*keepInterfaces=*/ strict));  
        if (dst == void.class)
            return dst;
        MethodHandle fn;
        if (src.isPrimitive()) {
     ...
     *    t5:L=MethodHandle.invokeBasic(t3:L,a1:L);         
     *    t6:L=MethodHandleImpl.loop(null,t2:L,t3:L);       
     *    t7:L=MethodHandle.invokeBasic(t4:L,t6:L);t7:L}    
     * }</pre></blockquote>
     * <p>
     ...
     *    t2:L=BoundMethodHandle$Species_LLLL.argL0(a0:L);  
     *    t3:L=BoundMethodHandle$Species_LLLL.argL1(a0:L);  
     *    t4:L=BoundMethodHandle$Species_LLLL.argL2(a0:L);  
     *    t5:L=BoundMethodHandle$Species_LLLL.argL3(a0:L);  
     *    t6:L=MethodHandle.invokeBasic(t4:L,a1:L);         
     *    t7:L=MethodHandleImpl.tryFinally(t2:L,t3:L,t6:L); 
     *    t8:L=MethodHandle.invokeBasic(t5:L,t7:L);t8:L}    
     * }</pre></blockquote>
     * <p>
