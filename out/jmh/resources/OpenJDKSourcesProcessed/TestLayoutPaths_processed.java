...
public class TestLayoutPaths {
    ...
    public void testByteOffsetHandleBadRange() {
        SequenceLayout seq = MemoryLayout.sequenceLayout(5, MemoryLayout.structLayout(JAVA_INT));
        seq.byteOffsetHandle(sequenceElement(5, 1)); 
    }
    ...
    public void testBadAlignmentOfRoot() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            assertEquals(seg.address() % JAVA_SHORT.byteAlignment(), 0); 
            assertNotEquals(seg.address() % struct.byteAlignment(), 0); 
    ...
    public static Object[][] groupSelectors() {
        return new Object[][] {
                { (IntFunction<PathElement>) PathElement::groupElement }, 
                { (IntFunction<PathElement>) i -> PathElement.groupElement(String.valueOf(i)) } 
        };
    }
