...
public class JMenu extends JMenuItem implements Accessible,MenuElement
{
    ...
    protected Point getPopupMenuOrigin() {
        ...
        if (position.y + y < screenBounds.y) { 
            y = screenBounds.y - position.y; 
        }
        if (position.y + y >= screenBounds.y + screenBounds.height) { 
            y = screenBounds.y + screenBounds.height - 1 - position.y; 
        }
        if (position.x + x < screenBounds.x) { 
            x = screenBounds.x - position.x; 
        }
        if (position.x + x >= screenBounds.x + screenBounds.width) { 
            x = screenBounds.x + screenBounds.width - 1 - position.x; 
        }
