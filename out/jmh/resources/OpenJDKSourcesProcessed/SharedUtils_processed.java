...
public final class SharedUtils {
    ...
    public static MethodHandle adaptDowncallForIMR(MethodHandle handle, FunctionDescriptor cDesc, CallingSequence sequence) {
        ...
        MethodHandle ret = identity(MemorySegment.class); 
        handle = collectArguments(ret, 1, handle); 
        handle = mergeArguments(handle, 0, 1 + imrAddrIdx);  
        handle = collectArguments(handle, 0, insertArguments(MH_ALLOC_BUFFER, 1, cDesc.returnLayout().get())); 
    ...
    private static MethodHandle adaptUpcallForIMR(MethodHandle target, boolean dropReturn) {
        ...
        target = collectArguments(MH_BUFFER_COPY, 1, target); 
