    ...
    void testStackTracker() {
        ClassFile.of().build(ClassDesc.of("Foo"), clb ->
            clb.withMethodBody("m", MethodTypeDesc.of(ConstantDescs.CD_Void), 0, cob -> {
                var stackTracker = CodeStackTracker.of(DoubleType, FloatType); 
        ...
        ClassFile.of().build(ClassDesc.of("Foo"), clb ->
            clb.withMethodBody("m", MethodTypeDesc.of(ConstantDescs.CD_Void), 0, cob -> {
                ...
                cob.transforming(stackTracker, stcb -> {
                    ...
                    assertFalse(stackTracker.stack().isPresent()); 
                    assertTrue(stackTracker.maxStackSize().isPresent()); 
                    var l2 = stcb.newBoundLabel(); 
                    assertFalse(stackTracker.stack().isPresent()); 
                    assertTrue(stackTracker.maxStackSize().isPresent()); 
                    stcb.constantInstruction(ClassDesc.of("Phee")); 
                    assertFalse(stackTracker.stack().isPresent()); 
                    assertFalse(stackTracker.maxStackSize().isPresent()); 
                    stcb.athrow();
                    stcb.labelBinding(l1); 
                    assertTrue(stackTracker.stack().isPresent()); 
                    assertFalse(stackTracker.maxStackSize().isPresent()); 
                    stcb.goto_(l2); 
                    assertFalse(stackTracker.stack().isPresent()); 
                    assertFalse(stackTracker.maxStackSize().isPresent()); 
