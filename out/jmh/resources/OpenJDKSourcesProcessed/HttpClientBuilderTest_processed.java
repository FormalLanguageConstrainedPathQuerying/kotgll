...
public class HttpClientBuilderTest {
    ...
    static final URI uri = URI.create("http:
    ...
    static class HttpConnectRequest extends HttpRequest {
        ...
        @Override public URI uri() { return URI.create("http:
    ...
    static void testDefaultClose() {
        ...
        try (HttpClient client = mock) { }
        assertEquals(shutdownCalled.get(), 1); 
        assertEquals(shutdownNowCalled.get(), 0); 
        assertEquals(awaitTerminationCalled.get(), 2); 
        assertFalse(Thread.currentThread().isInterrupted());

        try (HttpClient client = mock) { }
        assertEquals(shutdownCalled.get(), 3); 
        assertEquals(shutdownNowCalled.get(), 1); 
        assertEquals(awaitTerminationCalled.get(), 4); 
