...
public class ClassInitBarrier {
    ...
    static class Test {
        ...
        static void runTests() {
            checkBlockingAction(Test::testInvokeStatic);       
            checkBlockingAction(Test::testInvokeStaticSync);   
            checkBlockingAction(Test::testInvokeStaticNative); 
            checkBlockingAction(Test::testGetStatic);          
            checkBlockingAction(Test::testPutStatic);          
            checkBlockingAction(Test::testNewInstanceA);       

            checkNonBlockingAction(Test::testInvokeStaticJNI);       
            checkNonBlockingAction(Test::testInvokeStaticSyncJNI);   
            checkNonBlockingAction(Test::testInvokeStaticNativeJNI); 
            checkNonBlockingAction(Test::testGetStaticJNI);          
            checkNonBlockingAction(Test::testPutStaticJNI);          
            checkBlockingAction(Test::testNewInstanceAJNI);          

            A recv = testNewInstanceB(NON_BLOCKING.get());  
            checkNonBlockingAction(Test::testNewInstanceB); 

            checkNonBlockingAction(recv, Test::testGetField);      
            checkNonBlockingAction(recv, Test::testPutField);      
            checkNonBlockingAction(recv, Test::testInvokeVirtual); 

            checkNonBlockingAction(Test::testNewInstanceBJNI);        
            checkNonBlockingAction(recv, Test::testGetFieldJNI);      
            checkNonBlockingAction(recv, Test::testPutFieldJNI);      
            checkNonBlockingAction(recv, Test::testInvokeVirtualJNI); 
        }
    ...
    static void checkBlockingAction(TestCase0 r) {
        switch (phase) {
            case IN_PROGRESS: {
                ...
                checkBlocked(ON_BLOCK, ON_FAILURE, r); 
                break;
            }
            case FINISHED: {
                r.run(NON_BLOCKING.get()); 
            ...
            case INIT_FAILURE: {
                ...
                test.run(NON_BLOCKING.get()); 
                checkNotBlocked(test);        
                break;
            }
    ...
    static <T> void checkNonBlockingAction(T recv, TestCase1<T> r) {
        r.run(recv, NON_BLOCKING.get());                  
        checkNotBlocked((action) -> r.run(recv, action)); 
    }
