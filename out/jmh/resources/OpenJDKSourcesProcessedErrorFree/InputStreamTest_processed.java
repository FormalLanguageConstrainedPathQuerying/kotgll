    ...
    static void testSkip(Path tmpdir) throws IOException {
        ...
        try (OutputStream out = Files.newOutputStream(file)) {
            ...
            try (InputStream in = Files.newInputStream(file)) {
                ...
                assertTrue(in.skip(size/4) == size/4); 
                assertTrue(in.available() == 3*size/4);

                int b = in.read();
                assertTrue(b == blah[size/4]);
                assertTrue(in.available() == 3*size/4 - 1);
                assertTrue(in.skip(-1) == -1); 
                assertTrue(in.available() == 3*size/4);

                assertTrue(in.skip(-size/2) == -size/4); 
                assertTrue(in.available() == size);

                assertTrue(in.skip(5*size/4) == size); 
                assertTrue(in.available() == 0);

                assertTrue(in.skip(-3*size/4) == -3*size/4); 
                assertTrue(in.available() == 3*size/4);
                ...
                assertTrue(in.skip(-12) == -12); 

                assertTrue(in.skip(3*size/4) == 3*size/4); 
                assertTrue(in.available() == 0);

                assertTrue(in.skip(-size/2) == -size/2); 
                assertTrue(in.available() == size/2);

                assertTrue(in.skip(-size) == -size/2); 
                assertTrue(in.available() == size);

                assertTrue(in.skip(size/2) == size/2); 
                assertTrue(in.available() == size/2);

                assertTrue(in.skip(Long.MIN_VALUE) == -size/2); 
                assertTrue(in.available() == size);
