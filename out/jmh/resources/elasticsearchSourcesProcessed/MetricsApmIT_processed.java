...
public class MetricsApmIT extends ESRestTestCase {
    ...
    public ElasticsearchCluster cluster = ElasticsearchCluster.local()
        ...
        .setting("telemetry.agent.server_url", "http:
        .build();
        ...
        Consumer<String> messageConsumer = (String message) -> {
            ...
            if (isElasticsearchMetric(apmMessage)) {
                ...
                samples.entrySet().forEach(sampleEntry -> {
                    var assertion = sampleAssertions.get(sampleEntry.getKey());
                    if (assertion != null && assertion.test((Map<String, Object>) sampleEntry.getValue())) {
