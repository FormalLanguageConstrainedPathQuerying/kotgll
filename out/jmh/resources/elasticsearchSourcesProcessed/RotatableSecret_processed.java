...
public class RotatableSecret {
    ...
    private void checkExpired() {
        ...
        boolean expired = secrets.prior != null && secrets.priorValidTill.compareTo(Instant.now()) <= 0; 
        if (stampedLock.validate(stamp) == false) {
            stamp = stampedLock.readLock();
            needToUnlock = true;
            expired = secrets.prior != null && secrets.priorValidTill.compareTo(Instant.now()) <= 0; 
        }
        try {
            if (expired) {
                ...
                if (stampUpgrade == 0) {
                    ...
                    expired = secrets.prior != null && secrets.priorValidTill.isBefore(Instant.now()); 
                } else {
                    stamp = stampUpgrade;
                }
                needToUnlock = true;
